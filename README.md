...
To request autonomous work, open a [new SmactorIO task](https://github.com/leonbreukelman/richard-router/issues/new?template=smactorio-task.yml). The tracked source is the [SmactorIO task form](.github/ISSUE_TEMPLATE/smactorio-task.yml), and `CONTRIBUTING.md` provides the exact authorized-maintainer CLI and label-readback path. The form applies only `smactorio` for maintainer triage. After confirming that the task is bounded and low-risk, a maintainer adds `autonomy:ready` and `risk:low`; all three labels are required before SmactorIO can consider the issue. Labels do not bypass the remaining safety, review, secret-scanning, or CI gates.
